...
1. **Serielle Port Schnittstelle definieren.(USB = Universal Serial Bus.)**

   * Überprüfe das in der Windows-Geräteverwaltung unter „Anschlüsse (COM & LPT)“.
   * Beispiel: *"USB Serial Port (COM5)"*

2. **Du benötigst ein C-Compiler-Toolset**,  z.B. Visual Studio 
...
Öffne den Geräte-Manager und suche unter **"Anschlüsse (COM & LPT)"** nach deinem Gerät, z. B. **COM5**.
...
### 1. Plausibilität der eingelesen Werte

Ziel: Ziel ist es ein C Programm zu entwickeln, das den Sensorwert aus der Blackbox auf Tastendruck ausliest und in eine csv-Datei speichert. 


### Alfred Labortask Beschreibung
...
### Joe Labortask Beschreibung

Das Programm soll folgende Funktionalitäten haben:

- Datenaufnahme zur Kalibrierung des Sensors
- Kalibrierung des Sensors
- Einzelmessung/Automatische Messung des Sensors

Allgemeine Anforderungen:

- Der Modus kann über die CLI ausgewählt werden (Einzelmessung, Kalibrierung)

Anforderungen an den Datenaufnahme zur Kalibrierung des Sensors: 

- In diesem Modus soll der reale Abstandswert auf der Kommandozeile eingegeben werden und mit "Enter" bestätigt werden. 
- Gemessene und reale Abstandswerte sollen in einer csv-Datei gespeichert werden. Diese Messwert-csv soll auch wieder eingelesen werden können.
- Der reale Abstandswert aus der Kommandozeile und der vom Sensor gemessene Abstandswert soll gespeichert werden.
- Die Differenz zwischen gemessenem und realem Abstand soll auf der Kommandozeile ausgegeben werden und zusammen mit den Werten in der csv-Datei gespeichert werden.
- Der gemessene Abstand und der reale Abstand soll auf der Kommandozeile ausgegeben werden.
- Der Modus soll so lange laufen, bis ein spezieller Tastendruck (z.B. "q") erfolgt.
- Die Visualisierung der Soll-Ist-Abweichung soll in Excel durch den Import der csv-Datei erfolgen.

Anforderungen an Kalibrierung des Sensors mit einer Look-Up Tabelle:

- Die Kalibrierung soll auf Basis der Messwert-csv erfolgen.
...
TODO: Prüfen der Look-Up Tabelle mit manuellen Messungen.

Erstellen der Look-Up Tabelle und Testen der Plausibilität:
   ...
   - Eingelesenen Wert mit Programm verarbeiten und den berechneten Abstand ausgeben.
   - Anhand der händisch erstellten Tabelle eine Look-Up Tabelle programmieren.
